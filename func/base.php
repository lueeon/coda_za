...
if ( function_exists('register_sidebar') ){
    register_sidebar(array(
        'before_widget' => '<div class="widget hor_side">',
        'after_widget' => '</div>',
...
function iSaymeDocumentTitle(){
	...
	if( is_home() ){//首页
		...
		printf('搜索 &quot;%1$s&quot; 结果',attribute_escape(get_search_query()));
		...
		printf('%1$s 的存档', single_month_title(' ', false));
		echo  ' - ', get_bloginfo('name'), $page;
	}elseif( is_404() ){ //404 错误页面
		echo '错误 404. 没有找到, 我很抱歉', ' - ', get_bloginfo('name');
	}else{ //其他没有考虑到的情况
...
function iSaymeHeadItem(){
	echo
	'<link rel="shortcut icon" href="'.get_bloginfo('template_url').'/images/favicon.png" type="image/x-icon" />',"\n",
...
function google_analytics(){
	if($GLOBALS['coda_za']['enable_google_analytics']) {
		if(!$GLOBALS['coda_za']['exclude_admin_analytics'] || !current_user_can('manage_options')) {
			echo $GLOBALS['coda_za']['google_analytics_code'];
		}
	}
}
...
function iSaymeKeywordsAndDescription(){
	...
	// default
	$keywords = $GLOBALS['coda_za']['keywords'] ? $GLOBALS['coda_za']['keywords'] : '';
	$description = $GLOBALS['coda_za']['description'] ? $GLOBALS['coda_za']['description'] : get_bloginfo('description');
