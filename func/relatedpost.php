...
function iSaymeRelatedPosts( $args = '' ){
	...
	$default = array('limit'=>5, 'excerpt_length'=>83);
	...
	if(!$related_posts){
		...
		foreach($related_posts as $related_post){
			...
			$output .= '<li><a href="'.get_permalink($related_post).'" title="'.$post_title.'" rel="bookmark">'.$post_title.'</a>'.$comment_count.$post_excerpt.'</li>'."\n";
		}
	}
	$output .= '</ul>'."\n";
